...
async function checkPractitionerAccessToFamilyMemberRecords(
  ...
  try {
    const contract = await provideContract();
    const hasAccess =
      await contract.viewerHasAccessToPatientFamilyMemberMedicalRecord(
        practitionerAddress,
        patientId,
        familyMemberId,
        recordId
...
async function approveAccessToAddNewMedicalRecordForFamilyMember(
  ...
  try {
    const contract = await provideContract();
    const transaction =
      await contract.approveAccessToAddNewRecordForFamilyMember(
        doctorAddress,
        familyMemberId,
        principalPatientId
      );
...
async function approveFamilyMemberMedicalRecordAccess(
  ...
  try {
    ...
    switch (approvalType) {
      case "view":
        const readResult =
          await approveAccessToExistingFamilyMemberMedicalRecord(args);
      ...
      case "modify":
        const writeResult =
          await approveAccessToAddNewMedicalRecordForFamilyMember({
